...
class Main {
    ...
    private static List<Mark> loadMarksFromFile() {
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(FILE_NAME))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                marks.add(new Mark(Integer.parseInt(data[0]), data[1], Double.parseDouble(data[2]),
    ...
    private static void saveMarksToFile(List<Mark> marks) {
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(FILE_NAME))) {
            for (Mark mark : marks) {
                writer.write(mark.getRollNo() + DELIMITER + mark.getName() + DELIMITER +
                        mark.getsweMark() + DELIMITER + mark.getsnkMark() + DELIMITER + mark.getevsMark());
                writer.newLine();
            }
        } catch (IOException e) {
